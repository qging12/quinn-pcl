...
def main():
    ...
    #points_in = np.array(points_in, order='C', dtype=np.float32)
    ...
    #points_out = np.array(points_out, order='C', dtype=np.float32)
    cloud_out.from_array(points_out)
    #cloud_in = np.array(cloud_in, order='C')
    #cloud_out = np.array(cloud_out, order='C')
    ...
    #cloud_out = np.array(cloud_out, order='C')
    print('running')
    converged, transf, estimate, fitness = icp.icp(cloud_in, cloud_out, 10)
...
def pcl_icp(npSample, npReference):
    sampleCloud = pcl.PointCloud()
    referenceCloud = pcl.PointCloud()

    sampleCloud.from_array(npSample)
