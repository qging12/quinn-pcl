...
def change_detection(sampleCloud, referenceCloud, res):
    output = octree_cd.cd(sampleCloud, referenceCloud, res)
    return(output)
...
def main():
    ...
    parser.add_argument("-oc", "--octreeChange", action="store_true", help="Perform octree differencing")
    ...
    if args.output:
        file_out(sampleNP, "ply", args.output)
